...
function submit() {
  if (!form.value.nombre.trim()) return alert("Nombre obligatorio");
...
<template>
  <AdminLayout title="Gestión de Camas">
    ...
    <form @submit.prevent="submit" class="mb-6 flex gap-2">
      <input v-model="form.nombre" placeholder="Nombre de la cama" class="border p-2 flex-1 rounded"/>
      ...
      <tbody>
        <tr v-for="cama in camas" :key="cama.id" class="border-t">
          ...
          <td class="p-2 flex gap-2">
            <button @click="update(cama)" class="bg-green-500 text-white px-2 py-1 rounded">Guardar</button>
            <button @click="destroy(cama)" class="bg-red-500 text-white px-2 py-1 rounded">Eliminar</button>
          </td>
        </tr>
      </tbody>
    </table>
